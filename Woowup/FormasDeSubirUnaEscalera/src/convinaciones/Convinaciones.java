	...
	*/
	public String[] tiposDeConvinaciones(int escalonesASubir){
		...
		for(int cant2 = 0; cant2< CantMax2; cant2++){
			...
			int x = escalonesRestantes - (cant2 +2);
			...
			do{
				...
				if(ultimaPosicion && !(ubicacionDe2[cant2] == ubicacionDe2[cant2-1]-2)){
					for(int w = 0; w<cant2;w++) {
						ubicacionDe2[w]=ubicacionDe2[w]-x;
					}
					x--;
					vuelta--;
				}else {
				...
				while(escalones[ubicacionDe2[0]+1]==0 && 
						ubicacionDe2[0]-k == ubicacionDe2[k] &&
						escalonesASubir>k && cant2>1 &&
						convinacion != convinaciones && 
						k<cant2) {
					k++;
				}
				if(k>0 && ubicacionDe2[k]+2 == ubicacionDe2[k-1]) {
					cancelarVuelta = true;
				}else if(k==0 && cant2>1 && escalonesRestantes > 3 && 
